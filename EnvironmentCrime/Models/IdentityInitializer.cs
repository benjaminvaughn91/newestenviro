...
namespace EnvironmentCrime.Models
{
    public class IdentityInitializer
    {
        ...
        private static async Task CreateDummyUsers(UserManager<IdentityUser> uManager)
        {
            ...
            await uManager.AddToRoleAsync(E901, "Investigator");
            await uManager.AddToRoleAsync(E902, "Investigator");
            await uManager.AddToRoleAsync(E774, "Investigator");
            await uManager.AddToRoleAsync(E700, "Investigator");
        }

        private static async Task CreateAdmin(IServiceProvider services)
        {
            var uManager = services.GetRequiredService<UserManager<IdentityUser>>();
            IdentityUser A000 = new IdentityUser("A000");
            await uManager.CreateAsync(A000, "password");
            await uManager.AddToRoleAsync(A000, "Administrator");
        }
